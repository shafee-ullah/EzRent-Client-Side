...
const DashboardLayout = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await axios.post("https://ez-rent-server-side-seven.vercel.app/hostRequest", formData);
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="hidden md:flex items-center gap-4 mr-4">
              <div className="text-right border-r border-gray-200 dark:border-gray-700 pr-4">
                ...
                <p className="text-lg font-semibold text-gray-900 dark:text-white">{new Date().toLocaleDateString('en-US', { weekday: 'long' })}</p>
              </div>
                    ...
                    <div className="max-h-96 sm:max-h-96 max-sm:max-h-[60vh] overflow-y-auto">
                      {notifications.length > 0 ? (
                        notifications.map((notification) => (
                          <div
                            key={notification.id}
                            className={`p-4 border-b border-gray-100/50 dark:border-gray-700/50 last:border-b-0 hover:bg-gray-50/50 dark:hover:bg-gray-700/50 transition-colors ${!notification.read ? 'bg-blue-50/50 dark:bg-blue-900/20' : ''
                              }`}
                          >
